...
/*   Updated: 2026/04/08 20:03:46 by amyrodri         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	draw_header(t_data *game, int panel_x, int panel_y)
{
	...
	mlx_string_put(game->mlx, game->win, panel_x + 90, panel_y + 100, 0x00FF88, "ACTIVE");
	mlx_string_put(game->mlx, game->win, panel_x + 20, panel_y + 120, 0xAAAAAA, "Clearance:");
	mlx_string_put(game->mlx, game->win, panel_x + 120, panel_y + 120, 0xFF4444, "DENIED");
...
void	draw_sub_title(t_data *game, int panel_x, int panel_w, int panel_y)
{
	...
	mlx_string_put(game->mlx, game->win, panel_x + (panel_w / 2) - (text_width / 2), panel_y + 265, 0x445566, str);
}
...
void	draw_content(t_data *game, int panel_x, int panel_w, int panel_y)
{
	...
	mlx_set_font(game->mlx, game->win, "fixed");
	mlx_string_put(game->mlx, game->win, panel_x + 330, panel_y + 435, 0x445566, get_time_str());
	mlx_string_put(game->mlx, game->win, panel_x + 20, panel_y + 435, 0x445566, "FILE #0047");
...
void render_title(t_data *game)
{
	...
	draw_line_fancy(&game->frame, (game->frame.width - (panel_w - 30)) / 2, panel_y + 135, panel_w - 30);
	draw_line_fancy(&game->frame, (game->frame.width - (panel_w - 30)) / 2, panel_y + 415, panel_w - 30);
	draw_sprite_to_frame(game, &game->logo, (game->frame.width / 2) - (game->logo.width / 2), (game->frame.height / 2) - (game->logo.height / 2) - 30);
